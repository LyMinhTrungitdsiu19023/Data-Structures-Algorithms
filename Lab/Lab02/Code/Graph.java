...
public class Graph extends GeomObject {
    ...
    public static Point2D[] standardingGraphToLogic(Point2D[] points, double xMin, double xMax, double yMin,
            double yMax) {
        List<Point2D> listPoint = Arrays.asList(points);
        List<Point2D> standardizedPoints = listPoint.stream().map(
                point -> new Point2D(standardizing(point.getX(), xMin, xMax), standardizing(point.getY(), yMin, yMax)))
